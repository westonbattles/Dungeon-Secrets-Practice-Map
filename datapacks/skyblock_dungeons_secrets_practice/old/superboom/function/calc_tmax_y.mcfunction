# Calculate tMaxY (only called if dirY is non-zero)
# tMaxY = (next_boundary - ray_origin) / ray_direction

# Find next boundary coordinate (in block coords)
scoreboard players operation #temp voxel.math = #voxY voxel.math

# If stepping positive, next boundary is current block + 1
# If stepping negative, next boundary is current block
execute if score #stepY voxel.math matches 1 run scoreboard players add #temp voxel.math 1
...
scoreboard players operation #temp voxel.math -= #rayY voxel.math

# Divide by direction to get tMax
scoreboard players operation #dividend voxel.math = #temp voxel.math
scoreboard players operation #divisor voxel.math = #dirY voxel.math
function voxel_math:divide

# Store result
scoreboard players operation #tMaxY voxel.math = #output voxel.math
